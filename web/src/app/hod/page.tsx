...
export default function HodDashboard() {
  const [stats, setStats] = useState({ sections: 0, students: 0, teachers: 0, subjects: 0 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDepartmentName(deptRes.data.data?.name || "");

        const [s, st, t, sub] = await Promise.all([
          api.get(`/sections?department_id=${deptId}`),
          api.get(`/students?department_id=${deptId}`),
          api.get(`/users?role=teacher&department_id=${deptId}`),
          api.get(`/subjects?department_id=${deptId}`),
        ]);
        setStats({
          sections: s.data.data?.length || 0,
          students: st.data.data?.length || 0,
          teachers: t.data.data?.length || 0,
          subjects: sub.data.data?.length || 0,
        });
  ...
  const cards = [
    { label: "Sections", value: stats.sections, bg: "bg-violet-50", ring: "ring-violet-100", color: "from-violet-500 to-violet-600" },
    { label: "Students", value: stats.students, bg: "bg-emerald-50", ring: "ring-emerald-100", color: "from-emerald-500 to-emerald-600" },
    { label: "Teachers", value: stats.teachers, bg: "bg-amber-50", ring: "ring-amber-100", color: "from-amber-500 to-orange-500" },
    { label: "Subjects", value: stats.subjects, bg: "bg-blue-50", ring: "ring-blue-100", color: "from-blue-500 to-blue-600" },
  ];
  ...
  const actions = [
    ...
    { label: "Timetable", href: "/hod/timetable", color: "from-blue-500 to-blue-600" },
    { label: "Year Mgmt", href: "/hod/years", color: "from-purple-500 to-purple-600" },
  ];
      ...
      <div className="relative overflow-hidden rounded-3xl p-8 lg:p-10 text-white animate-fade-in-up"
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/15 backdrop-blur-sm rounded-full border border-white/20 mb-6">
            ...
            <span className="text-xs font-medium text-white/80">Department View</span>
          </div>
          <h1 className="text-3xl lg:text-4xl font-extrabold mb-3 tracking-tight text-left">{departmentName || "HOD Dashboard"}</h1>
          <p className="text-white/70 text-lg text-left">Manage your department sections, teachers, students, and timetable</p>
        </div>
      </div>
